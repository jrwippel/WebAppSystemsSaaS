...
namespace ClockTrack.Controllers
{
    ...
    public class MensalistaController : Controller
    {
        ...
        public async Task<IActionResult> DownloadReport(string monthYearString, int? clientId, int? departmentId, string recordType = null, string format = "xlsx")
        {
            ...
            {
                activityTypeId = int.Parse(recordType);
            }

            var filteredRecords = await _processRecordService.FindByDateAsyncRes(minDate, maxDate, clientId, departmentId, activityTypeId);
