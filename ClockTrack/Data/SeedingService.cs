...
namespace ClockTrack.Data
{
    public class SeedingService
    {
        ...
        private ClockTrackContext _context;
        public SeedingService(ClockTrackContext context)
        {
            _context = context;     
        }
        ...
        public void Seed()
        {
            ...
            // Associar dados existentes ao tenant padrão (TenantId = 1)
            // Usar IgnoreQueryFilters para pegar registros com TenantId = 0
            var existingAttorneys = _context.Attorney.IgnoreQueryFilters().Where(a => a.TenantId == 0).ToList();
            var existingClients = _context.Client.IgnoreQueryFilters().Where(c => c.TenantId == 0).ToList();
            var existingDepartments = _context.Department.IgnoreQueryFilters().Where(d => d.TenantId == 0).ToList();
            var existingProcessRecords = _context.ProcessRecord.IgnoreQueryFilters().Where(p => p.TenantId == 0).ToList();

            if (existingAttorneys.Any() || existingClients.Any() || existingDepartments.Any() || existingProcessRecords.Any())
            {
                foreach (var attorney in existingAttorneys)
                    attorney.TenantId = 1;
                ...
                foreach (var processRecord in existingProcessRecords)
                    processRecord.TenantId = 1;

                _context.SaveChanges();
            }

            if (_context.ProcessRecord.IgnoreQueryFilters().Any() ||
                _context.Department.IgnoreQueryFilters().Any() ||
                _context.Attorney.IgnoreQueryFilters().Any())              
            ...
            */
            ...
            var defaultDepartment = new Department("Administrativo");
            defaultDepartment.TenantId = 1; // Associar ao tenant padrão
            _context.Department.Add(defaultDepartment);
            _context.SaveChanges();
