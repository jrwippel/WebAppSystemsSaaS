...
namespace ClockTrack.Services
{
    public class ProcessRecordService
    {
        ...
        public ProcessRecordService(ClockTrackContext context)
        {
            _context = context;
        }
    ...
    int? activityTypeId)
        {
            ...
            {
                result = result.Where(x => x.ActivityTypeId == activityTypeId.Value);
            }
        ...
        public async Task<List<ProcessRecord>> FindByDateAsyncRes(DateTime? minDate, DateTime? maxDate, int? clientId, int? attorneyId, int? activityTypeId)
        {
            var result = from obj in _context.ProcessRecord select obj;
            if (minDate.HasValue)
            {
