...
namespace ClockTrack.Services
{
    public class ProcessRecordsService
    {
        ...
        public ProcessRecordsService(ClockTrackContext context)
        {
            _context = context;
        }
        ...
        public async Task<DashboardKpis> GetDashboardKpis(int userId, ProfileEnum perfil)
        {
            var now = DateTime.Now;
            var today = now.Date;
            var yesterday = today.AddDays(-1);
            var firstDayMonth = new DateTime(now.Year, now.Month, 1);
            var firstDayLastMonth = firstDayMonth.AddMonths(-1);
            ...
            double horasHoje = all.Where(p => p.Date.Date == today).Sum(p => (p.HoraFinal - p.HoraInicial).TotalHours);
            double horasOntem = all.Where(p => p.Date.Date == yesterday).Sum(p => (p.HoraFinal - p.HoraInicial).TotalHours);
            double horasMes = all.Where(p => p.Date >= firstDayMonth).Sum(p => (p.HoraFinal - p.HoraInicial).TotalHours);
            double horasMesPassado = all.Where(p => p.Date >= firstDayLastMonth && p.Date <= lastDayLastMonth).Sum(p => (p.HoraFinal - p.HoraInicial).TotalHours);
            int registrosHoje = all.Count(p => p.Date.Date == today);
            int clientesAtivos = await _context.Client.CountAsync(c => !c.ClienteInativo);

            return new DashboardKpis
            {
                HorasHoje = Math.Round(horasHoje, 1),
                HorasOntem = Math.Round(horasOntem, 1),
                HorasMes = Math.Round(horasMes, 1),
                HorasMesPassado = Math.Round(horasMesPassado, 1),
